<?php 
$page_title="Award >> Text Management";
include('include/login_header.php');
$data=$hobj->select("*","wm_award","1");
//print_r($data);
if(isset($_POST['submit']))
{
	if(empty($_REQUEST['editor1']))
			...
			else
			{
					...
					$award_text=$_REQUEST['editor1'];
					
					$id=$_REQUEST['tdaid'];
							
					$updatedata=$hobj->update("wm_award","wm_awardtext","'$award_text'","wm_awardid='$id'");
				...
				//$insertquery=$hobj->insert("tda_hometxt","home_wel_title,home_wel_text,home_ch_head,home_ch_text,home_pr_head,home_pr_text,home_sch_text","'$wel_title','$wel_text','$ch_head','$ch_text','$pr_head','$pr_text','$sch_text'");
					...
					{?>
							<script>
							alert("Award Page Text Content Update Successfully.");
							</script>
					<?php
						echo '<script>window.location="award.php"</script>';
					}
					else
					{?>
								<script>
...
<!--  start page-heading -->
	...
	<tr>
		...
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form">
			...
			<tr>
				<th valign="top">Award Content:</th>
				<td>				
