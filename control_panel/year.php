<?php 
$page_title="Year Management >>Year Category";
include('include/login_header.php');

$data=$hobj->select("wm_year,wm_subname,wm_name,wm_yearstatus","wm_year,wm_cat,wm_subcat","wm_subcatid_year=wm_subcatid AND wm_catid_year=wm_catid AND wm_catid_sub=wm_catid order by wm_year");
/* on form submit*/
...
{	
	...
	if(!empty($_POST['year'])) 
	{
		...
		$year=$_POST['year'];
		$status=$_POST['status'];
		$insert_query=$hobj->insert("wm_year","wm_catid_year,wm_subcatid_year,wm_year,wm_yearstatus","'$cat_type','$subcat_id','$year','$status'");
		
		if($insert_query)
		{?>
				<script>alert('Year Added Successfully')</script>
		<?php 
	...
	<tr>
		...
		<table border="0" cellpadding="0" cellspacing="0"  id="id-form" >
			...
			<tr>
				...
				<td>
				<div id="subcatdiv">
					<?php $hobj->dropdown("wm_subcatid,wm_subname","wm_subcat","wm_substatus='Enable' AND wm_subcatid='1'","subcat_type","styledselect_form_5"); ?></div>&nbsp;<span id="s_sname"></span>
				</td>
			</tr>
			<tr>
				...
				<td>
					...
					<?php $hobj->text("text","year","inp-form","Enter Year","","");?>&nbsp;<span id="s_sname"></span>
				</td>
			</tr>
			<tr>
