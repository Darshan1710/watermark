<?php 
$page_title="Careers >> Text Management";
include('include/login_header.php');
$data=$hobj->select("*","wm_career","1");
//print_r($data);
if(isset($_POST['submit']))
{
	if(empty($_REQUEST['editor1']))
			...
			?>
						<script>
							alert("Please Enter Careers Content");
						</script>
			<?php
			}
			else
			{
					...
					$career_text=$_REQUEST['editor1'];
					
					$id=$_REQUEST['tdaid'];
							
					$updatedata=$hobj->update("wm_career","wm_careertext","'$career_text'","wm_careerid='$id'");
				...
				//$insertquery=$hobj->insert("tda_hometxt","home_wel_title,home_wel_text,home_ch_head,home_ch_text,home_pr_head,home_pr_text,home_sch_text","'$wel_title','$wel_text','$ch_head','$ch_text','$pr_head','$pr_text','$sch_text'");
					...
					{?>
							<script>
							alert("Careers Page Text Content Update Successfully.");
							</script>
					<?php
